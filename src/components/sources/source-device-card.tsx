        ...
        <div className="relative h-32 w-48 flex-shrink-0">
          ...
          {(status !== 'COMPLETED' || showCompletedBadge) && (
            <div className="absolute top-2 left-2">
              <div
                className={`rounded-full ${getStatusColor(status)} text-foreground px-2 py-1 text-xs font-medium`}
              >
                {status}
              </div>
            </div>
    ...
    >
      ...
      {(status !== 'COMPLETED' || showCompletedBadge) && (
        <div className="absolute top-3 left-3">
          <div
            className={`rounded-full ${getStatusColor(status)} text-foreground px-3 py-1 text-xs font-medium transition-opacity duration-300`}
          >
            {status}
          </div>
        </div>
      ...
      {isProcessing && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex flex-col items-center gap-2 rounded-lg bg-black/70 px-6 py-4 backdrop-blur-sm">
            ...
            {processingStep && <p className="text-foreground text-xs">{processingStep}</p>}
            {processingProgress !== null && processingProgress !== undefined && (
              <p className="text-muted-foreground text-xs">{processingProgress}%</p>
            )}
          </div>
        </div>
      )}
